...
双击桌面现有的 **Humanity Trinity Space** 快捷方式，即可在同一个三维教室窗口里选择“漫游”或“捉迷藏”。可以点击画面中的模式卡片，也可以按 `1` 选择漫游、按 `2` 选择捉迷藏；游玩期间按 `M` 返回选择画面。

![教室中的游玩方式选择画面](Docs/Previews/HumanityTrinityRebuild_ModeMenu.png)

选择捉迷藏后，前 12 秒开灯记住教室，随后在完全黑暗中，用三分钟寻找一名电脑控制的藏者。建议戴耳机分辨脚步方向。
...
| 按住 F | 连续摸索约一臂内的物体，同时减速；摸到人物真实姿态中的身体才算抓获 |
| E | 操作手边的舞台暗门 |
| Tab | 重开为明亮练习模式，再按切回黑暗；练习时能观察藏者的动作和当前行为 |
| R / Esc | 重开一局 / 退出 |
| M | 回到漫游与捉迷藏的选择画面 |
...
当前范围为一名电脑藏者和教室平地上的藏点。蹲伏用于缩小身形和降低接触高度；尚未实现钻到桌底、攀爬、舞台道具间藏点、双人联机、第一人称手臂或手柄震动。人物采用分部件关节动画，后续可以进一步换成蒙皮角色。原来的漫游关卡独立保留。开发前的完整建模版本固定为远端标签 `v1.0.0-reconstruction`。
