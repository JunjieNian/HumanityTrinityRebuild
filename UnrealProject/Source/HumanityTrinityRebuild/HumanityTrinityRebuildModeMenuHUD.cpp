...
FBox2D AHumanityTrinityRebuildModeMenuHUD::GetChoiceBox(int32 Choice) const
{
    ...
    const float Y = ViewSize.Y * (0.35f + Choice * 0.16f);
...
bool AHumanityTrinityRebuildModeMenuHUD::SelectAt(float X, float Y)
{
    ...
    if (GetChoiceBox(2).IsInside(FVector2D(X, Y)))
    {
...
void AHumanityTrinityRebuildModeMenuHUD::DrawHUD()
{
    ...
    DrawText(TEXT("Explore the room. Choose a side in the dark."), FLinearColor(.78f, .83f, .81f), Left, H * 0.27f,
             GEngine->GetMediumFont(), 0.95f, false);

    float MouseX = -1, MouseY = -1;
    if (auto* Controller = GetOwningPlayerController())
        Controller->GetMousePosition(MouseX, MouseY);
    const TCHAR* Titles[] = {TEXT("1   WALK THROUGH THE SPACE"), TEXT("2   YOU SEEK / NPC HIDES"),
                            TEXT("3   YOU HIDE / NPC SEEKS")};
    const TCHAR* Descriptions[] = {TEXT("Explore the lit room at your own pace"),
        TEXT("Find the hidden person by sound and touch"),
        TEXT("Choose a hiding place. Stay quiet. Slip away.")};
    for (int32 Choice = 0; Choice < 3; ++Choice)
    {
        ...
        DrawText(Titles[Choice], FLinearColor::White,
                 Box.Min.X + 24, Box.Min.Y + 14, GEngine->GetMediumFont(), 1.15f, false);
        DrawText(Descriptions[Choice],
