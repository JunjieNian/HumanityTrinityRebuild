...
双击桌面现有的 **Humanity Trinity Space** 快捷方式，即可在同一个三维教室窗口里选择玩法。点击模式卡片，或按 `1` 选择漫游、`2` 选择“你寻找，NPC 躲藏”、`3` 选择“你躲藏，NPC 寻找”；游玩期间按 `M` 返回选择画面。

![教室中的游玩方式选择画面](Docs/Previews/HumanityTrinityRebuild_ModeMenu.png)

选择“你寻找”后，前 12 秒开灯记住教室，随后在完全黑暗中，用三分钟寻找一名电脑控制的藏者。选择“你躲藏”后，有 20 秒选择藏身位置，也可以按空格提前准备好；关灯后坚持三分钟不被 NPC 摸到即可获胜。建议戴耳机分辨脚步方向。

| 操作 | 效果 |
|---|---|
| WASD、鼠标 | 移动、转向；抬头和低头会改变摸索方向 |
| 按住 Shift | 慢行，脚步传播范围明显缩小 |
| 按住 Ctrl | 蹲伏，视点降低、速度更慢，脚步更轻 |
| 按住 F | 连续摸索约一臂内的物体，同时减速；寻找时摸到藏者获胜，躲藏时摸到寻找者会暴露自己 |
| E | 操作手边的舞台暗门 |
| 空格 | 在“你躲藏”的准备阶段提前开始搜索 |
| Tab | 保留当前角色，重开为明亮练习模式；再按切回黑暗，可以观察 NPC 的动作和当前行为 |
| R / Esc | 重开一局 / 退出 |
| M | 回到漫游与捉迷藏的选择画面 |

### 藏者的外形和行为

藏者采用原创风格化人物：连帽衫、袋口、抽绳、衣物缝线、面部、头发、手指、长裤和运动鞋。九种模型部件组成十五段可动身体；站立时呼吸，警觉时转头，走路时迈步和摆臂，停止躲藏时弯膝蹲伏。完整可编辑模型在 `Assets/Hider/Trinity_Hider.blend`，生成脚本在 `Tools/Blender/build_hider.py`。

开局会检查桌椅间能够通行的路线，并从多个分散、有遮挡的藏点中选择位置。静止的藏者可以一直保持安静。它依据听到的脚步声判断威胁：声音受距离、行走方式和遮挡影响；听到附近动静后先停顿聆听，再决定是否转移。转移时绕过家具，避开记忆中的声音位置，并降低再次选择最近藏点的倾向。它移动时也会发出可定位的脚步，玩家可以利用这个代价追踪它。

触摸反馈来自接触到的材质和高度，包括木面、金属、布料、塑料等；未知材质显示普通硬质表面。桌椅和墙体会先挡住手，不会隔着障碍抓到人。视点低下来后可以摸更低的位置。触感目前通过短文字和空间声音替代，尚未接入真实触觉设备。

### 你躲藏，NPC 寻找

准备阶段 NPC 面向教学墙等待，不会听取或跟随你的躲藏过程。开始搜索后，它会巡查尚未检查的区域，听到脚步时先停下辨认，再绕开桌椅前往声音的大致位置；到达后转身伸手，并蹲下检查低处。找不到人时会检查附近位置，然后继续巡查。你之后悄悄移动，不会直接改变它记住的目标。

NPC 依靠声音事件和短距离实际触碰来寻找。正常走路较容易被听见，按住 Shift 慢行或 Ctrl 蹲伏可以明显缩小声音传播范围；静止时不产生脚步。墙和家具会减弱声音、挡住伸出的手。你可以听 NPC 的空间脚步声判断它正在接近还是远离。

这一玩法的场地为教室桌椅区，舞台前的临时栏杆标记边界，防止进入 NPC 当前不能搜索的舞台和道具间。被找到或坚持到计时结束都会恢复照明，按 R 重开即可再选一个藏身位置。第一次建议先按 Tab 开灯练习，再挑战全黑环境。

当前范围为一名电脑对手和教室平地上的藏点，可以任选寻找或躲藏。蹲伏用于缩小身形和降低接触高度；尚未实现钻到桌底、攀爬、舞台道具间藏点、双人联机、第一人称手臂或手柄震动。人物采用分部件关节动画，后续可以进一步换成蒙皮角色。原来的漫游关卡独立保留。开发前的完整建模版本固定为远端标签 `v1.0.0-reconstruction`。
